...
func (r *repo) GetByCategoryID(c context.Context, categoryID uuid.UUID) ([]*menu.Dish, error) {
	output := []*menu.Dish{}
	...
	sql := `SELECT
					...
					WHERE d.category_id = $1 ORDER BY d.position ASC`
	...
	dishMap := map[uuid.UUID]*menu.Dish{}
